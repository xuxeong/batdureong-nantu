// 튜토리얼 화면 (DEC-UI-030, DEC-RUN-003, DEC-CONTENT-025)
//
// ── 지금은 껍데기다. 그 사실을 화면이 말한다 ───────────────
//
// `DEC-UI-030` 가 **안내 문구를 코드에 두지 못하게** 확정했다 —
...
//
// ── 데이터가 오면 채울 것 (DEC-UI-030) ─────────────────────
//
//   - 재배 → 전투 → 정비 순서로 안내하고, 각 안내는 해당 조작을 **실제로 성공하면**
//     다음으로 넘어간다 (`completion_key` 일곱 개를 코드가 판정한다, DEC-CONTENT-025)
...
export interface TutorialHandlers {
  /**
   * 튜토리얼을 건너뛰고 1일차로 간다 (DEC-UI-030).
   *
   * `DEC-UI-030` 는 "타이틀 또는 튜토리얼 시작 시점에 제공한다" 로 둘 중 하나를
