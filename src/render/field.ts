...
export interface PlotView {
   ...
   * 한 장이고(작물별로 두지 않는다 — `DEC-ART-003`) 성장·수확 가능은 작물 행의
 ...
 * `DEC-ART-003` 이 월드 1단위 = 화면 1픽셀로 정해서 **에셋 크기가 곧 화면 크기다.**
...
export interface FieldView {
   ...
   * 없으면 사각형으로 그린다.
   *
   * **캔버스 좌우 반전은 쓰지 않는다.** `DEC-ART-003` 가 좌·우를 `field_sprite_left`·
   ...
   * `DEC-ART-001` 폐기·`DEC-ART-003` 대체로 그 길이 없어졌다.
   */
  playerAsset?: string | null
  /**
   * 걷는 흔들림의 위상 0~1. 멈춰 있으면 null (`DEC-ART-003`).
   *
   ...
   * 말고는 맞았다는 신호가 없었다. `DEC-ART-003` 가 허용한 투명도로 깜빡인다.
   */
  playerHit?: number
}
...
export interface HostileView {
  ...
  assetId?: string | null
  /**
   * 걷는 흔들림의 위상 0~1. 멈춰 있거나 대상이 아니면 null (`DEC-ART-003`).
   *
   * **적대 주민에게만 온다.** `DEC-ART-003` 이 *"야생동물에는 bob 을 적용하지
   * 않는다"* 로 확정했고, 야생동물은 대신 아래 `heading` 으로 진행 방향을
   * 향해 회전한다. 부르는 쪽이 주민 자리에서만 채운다 (main.ts `hostileViews()`).
   */
  bob?: number | null
  /**
   * 진행 방향(라디안). 회전하지 않으면 null (`DEC-ART-003`).
   *
   ...
   *
   * 회전은 `DEC-ART-003` 까지 허용 수단(위치·크기·투명도)에 없었고
   * `DEC-ART-003` 이 야생동물에 한해 더했다. 사람에게는 쓰지 않는다.
   */
  heading?: number | null
  /**
 ...
 * **표현이라 승인 데이터가 아니다.**
 *
 * `DEC-ART-003` 가 *"걷기 동작은 예외에 넣지 않는다. 이동 중 흔들림은 예외가
...
export function createFieldRenderer(
   ...
   *
   * `bob` 이 0~1 위상으로 오면 걷는 흔들림을 얹는다 (`DEC-ART-003`). null 이면
  ...
  function drawPlot(plot: PlotView, assets: FieldAssetIds | undefined): void {
    ...
    // 씨앗은 작물별로 두지 않고 맵에 한 장이라(DEC-ART-003) 여기서 작물 ID 로
