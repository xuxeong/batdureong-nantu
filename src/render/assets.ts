// 논리 에셋 ID 해석과 이미지 적재 (DEC-ART-002, AGENTS.md 6절)
//
// ── 경로를 아는 곳은 여기 하나다 ───────────────────────────
//
 ...
 * 고정 목록이 유일한 자리다 (`DEC-ART-002`, 아트 디렉션 14.3). 코드가 쓰는 ID 를
 ...
 * 변경 가능한 게임 데이터이고, 이 ID 는 `DEC-ART-002` 이 확정한 고정 목록의 값이라
...
for (const id of Object.values(UI_ASSET)) {
  ...
  throw new Error(
    `${id} 가 schema/enums.json 의 ui_system_asset_id 고정 목록에 없다. ` +
      '새 ID 추가는 데이터 행 추가가 아니라 스키마 변경이다 (DEC-ART-002)',
  )
}
 ...
 * 원본이지만(`DEC-ART-002`, 8/4) 파일 경로까지 갖게 하면 경로를 아는 곳이 둘이 되고,
