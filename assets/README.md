...
규격과 파이프라인은 `DEC-ART-002`이 정한다. 이 문서는 폴더 배치만 설명한다.
...
이미지 PNG(투명 배경), 소리 MP3, 폰트 WOFF2. **폰트 라이선스가 파일 수정을 금지하면
원본 형식을 그대로 둔다** — 형식 변환과 서브셋이 그 금지에 걸린다. 지금 쓰는
GriunXHangeul Equal 이 그 경우라 TTF 로 들어와 있다(`docs/submission/CREDITS.md`). 1차 프로토타입에서는 프레임 애니메이션과
