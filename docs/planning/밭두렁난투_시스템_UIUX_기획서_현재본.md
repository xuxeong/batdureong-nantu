...
핵심 근거: `DEC-FARM-001~006`, `DEC-FARM-010`, `DEC-CONTENT-003`, `DEC-CONTENT-007`, `DEC-ART-004`.
...
전부 보여야 하며, `DEC-ART-004`이 그 제약 안에서 **월드 1단위 = 화면 1픽셀**로 확정했다.
...
세 종류는 시각적으로 구분하되 실제 색과 표현은 `DEC-ART-004`에서 정한다.
...
**폐기되지 않은 `DEC-UI-*` 30개와 아트 규격(`DEC-ART-004`)이 모두 확정됐다.** 남은 것은 화면 문구 테이블
...
논리 좌표를 그 중심에 맞춘다. 이미지는 PNG, 소리는 MP3, 폰트는 WOFF2를 쓰고, 1차
프로토타입에서는 **프레임 애니메이션과 스프라이트 시트를 쓰지 않는다.**

논리 에셋 ID는 `asset.<구간>.<이름>`이고 파일 경로는 카탈로그 없이
`assets/final/<구간>/<이름>.<확장자>`로 해석한다. 콘텐츠에 붙는 구간은 열세 가지이고, 어떤
콘텐츠에도 속하지 않는 UI·시스템 에셋은 `schema/enums.json`의 고정 허용 목록으로 관리한다.
해석되지 않는 ID는 플레이스홀더로 그린다.
