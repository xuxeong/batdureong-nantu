...
핵심 근거: `DEC-FARM-001~006`, `DEC-FARM-010`, `DEC-CONTENT-003`, `DEC-CONTENT-007`, `DEC-ART-003`.
...
전부 보여야 하며, `DEC-ART-003`이 그 제약 안에서 **월드 1단위 = 화면 1픽셀**로 확정했다.
...
세 종류는 시각적으로 구분하되 실제 색과 표현은 `DEC-ART-003`에서 정한다.
...
타이틀에는 새 런 시작 입력만 둔다. 이어하기와 저장 슬롯은 없다. **이름 입력은 키보드로 하며
...
핵심 근거: `DEC-UI-030`, `DEC-UI-016`, `DEC-UI-028`.
...
**폐기되지 않은 `DEC-UI-*` 30개와 아트 규격(`DEC-ART-003`)이 모두 확정됐다.** 남은 것은 화면 문구 테이블
