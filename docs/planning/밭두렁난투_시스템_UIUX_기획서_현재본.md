...
전부 보여야 하며, `DEC-ART-002`이 그 제약 안에서 **월드 1단위 = 화면 1픽셀**로 확정했다.
...
세 종류는 시각적으로 구분하되 실제 색과 표현은 `DEC-ART-002`에서 정한다.
...
**`DEC-UI-*` 28개와 아트 규격(`DEC-ART-002`)이 모두 확정됐다.** 남은 것은 화면 문구 테이블
