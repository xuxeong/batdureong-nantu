...
재배·습격 단계에서는 플레이어 중심부터 마우스 커서 방향으로 조준선을 표시한다. 길이는
플레이어 충돌 반경의 4.5배이며 붉은색 대신 어두운 갈색 외곽선과 밝은 한지색 안쪽 선을
겹쳐 쓴다. 필드가 입력을 소유하지 않으면 마지막 방향에서 멈춘다.
...
핵심 근거: `DEC-RESOURCE-001~005`, `DEC-RESOURCE-015~019`.
...
야생동물은 재배 단계에 출현한다. `crop_first`는 수확 가능 작물을 우선 노리고, `player_only`는 플레이어만 노린다. 씨앗과 성장 중 작물은 공격하지 않는다. 탑뷰 야생동물에는 bob을 적용하지 않고 기존 한 장짜리 스프라이트를 이동 벡터 방향으로 회전해 머리가 진행 방향을 향하게 한다. 멈추면 마지막 이동 방향을 유지한다. 재배 단계 종료 시 살아 있는 야생동물과 적용 효과를 제거한다.

핵심 근거: `DEC-FARM-001~006`, `DEC-FARM-010`, `DEC-CONTENT-003`, `DEC-CONTENT-007`, `DEC-ART-005`.
...
공포도는 전역 정수다. 위협은 1, 퇴각 대가 요구는 3, 처치는 6을 추가한다. 세 값의 원본은 `fear_increments.csv`이고 판정 구간(온기 0~4 / 경계 5~11 / 공포 12 이상)의 원본은 `fear_bands.csv`다. 제출 빌드에서는 수치를 직접 표시하지 않는다.
...
**동시에 열리는 오버레이는 기능 오버레이 하나와 일시정지 하나뿐이다.** 지금 규칙상 그 이상이
나오지 않는다 — 전투 전 대화와 투항 대화는 이어질 수 없고(`DEC-RESIDENT-015`), 대화가 시작되면
회복 사용이 취소되며(`DEC-CONTENT-006`), 정비와 대화는 단계가 다르다. 일시정지는 항상 가장
위에 온다. 정비 허브 안의 팝업은 오버레이 층이 아니라 허브 내부 화면이다.

가장 위 오버레이가 입력을 독점하고 아래 층위는 표시만 된다. 배경 처리는 각 오버레이의 확정
규칙을 따르되 겹치면 **더 강한 정지가 이긴다** — 회복 퀵메뉴의 감속 위에 일시정지가 오면
완전히 멈추고 딤을 건다.

`Esc` 한 번은 한 층만 처리한다. 회복 퀵메뉴가 열려 있으면 그것만 닫고, 없으면 일시정지를
열거나 닫는다. 포커스를 잃어 일시정지가 자동으로 열릴 때 퀵메뉴가 떠 있으면 퀵메뉴를 닫되
선택한 회복 아이템은 유지한다.
...
기준 해상도는 16:9의 **1920×1080**이다. 승인된 맵의 `world_width`·`world_height`가 그 안에
전부 보여야 하며, `DEC-ART-005`이 그 제약 안에서 **월드 1단위 = 화면 1픽셀**로 확정했다.
맵이 화면보다 작아 생기는 여백은 필드 HUD가 쓸 수 있다.

창 크기가 다르면 화면 전체를 비율을 유지한 채 확대·축소하고 남는 영역은 여백으로 둔다. 필드와
UI를 서로 다른 비율로 늘리지 않는다. **최소 브라우저 크기를 정하지 않고** 창이 작아도 플레이를
막거나 경고하지 않는다 — 제출 요건이 링크 클릭만으로 바로 플레이되는 것이기 때문이다.

1차 프로토타입의 **화면 조작은 마우스로만** 한다. 대화 선택지, 정비 허브 버튼, 보관함과 퀵슬롯
편성이 모두 그렇다. 선택지 번호키와 방향키 포커스 이동은 지원하지 않는다 — 화면에 조작 번호를
띄우지 않기 위한 선택이다. **필드 조작의 키보드 입력(`WASD`·`E`·`1~4`·`Q`·`Esc`)은
`DEC-INPUT-001~009` 그대로 유지된다.**

핵심 근거: `DEC-UI-025`.

### 필드 공통 HUD

재배·습격 모드 공통으로 체력(게이지+수치), 현재 일차, 투척 퀵슬롯 4칸, 선택된 회복
...
세 종류는 시각적으로 구분하되 실제 색과 표현은 `DEC-ART-005`에서 정한다.
...
조우 결과·밤 결과·런 실패·엔딩은 **각각 독립 화면이고 공유 템플릿으로 묶지 않는다**. 조우
결과와 밤 결과는 같은 층위라 하루에 하나만 뜨고, 각각 다음 일차로 넘어가는 진행 입력을 하나만
둔다.

조우 결과에는 주민 이름, 해결 방식, 최종 생존·소속·관계 상태를 표시한다. 보상은 **런타임 지급
기록을 기준으로 실제 지급된 것만** 보여주고, 보상이 없는 결과에는 영역 자체를 두지 않는다.
자원 협상이 성립했으면 실제로 소비된 작물과 수량을, 거절됐으면 소비되지 않았음을 표시한다.
사연 정보는 **이번 조우에서 새로 확인한 것만** `ending_fact_text`로 보여주며 `source_fact_text`
는 화면에 올리지 않는다. 영입한 주민이 다음 습격부터 지원한다는 사실도 여기서 전한다.

**공포도는 제출 빌드의 어떤 화면에도 수치·구간 이름·변화량을 표시하지 않는다.** 조우 결과에
공포도용 문장이나 게이지도 두지 않는다 — 관계 상태(우호·거래·동료·강압·단절)가 공포도를
올리는 행동과 대응하므로 같은 내용을 두 번 말하게 된다. 누적된 흐름은 매 일차 시작의 플레이어
일지가 전담한다. 개발 빌드에서는 수치와 변화량을 그대로 표시할 수 있다.

밤 결과에는 **승인된 고정 문구만** 올린다. 그날의 수확량이나 자원 변화 같은 실제 기록을 넣지
않는다. 런 실패 화면은 실패 사실과 도달 일차, 타이틀 복귀만 두고 엔딩 제목·요약·기록문·대표
작물·런 통계를 붙이지 않는다 — 체력 0으로 실패한 런에는 엔딩 판정도 기록문 생성도 실행되지
않기 때문이다(`DEC-CONTENT-011`). 엔딩 화면은 제목과 확정된 요약, 생성된 기록문을 보여주고
다시 생성하는 입력을 두지 않는다.

핵심 근거: `DEC-UI-011`, `DEC-UI-013`, `DEC-UI-023`.

### 시작 구간과 일차 시작

타이틀에는 새 런 시작 입력과 음량을 조절하는 설정 입력만 둔다. 이어하기, 저장 슬롯, 게임
종료 입력은 없다 — 설정 입력은 `DEC-UI-027`이 정한 음량 조절 항목만 열고 새 설정 화면을
만들지 않는다. **이름 입력은 키보드로 하며
마우스 전용 규칙의 예외다.** 비어 있으면 진행할 수 없고 최대 길이를 제한하며 줄바꿈을 받지
않는다. 튜토리얼은 재배 → 전투 → 정비 순서로 안내하고 각 조작을 실제로 성공해야 넘어간다.
시간 제한은 없고 전체 건너뛰기를 제공하며, 튜토리얼 자원이 본 런에 반영되지 않는다는 것을
끝에 알린다. **튜토리얼 안내 문구도 코드가 아니라 콘텐츠 데이터에서 온다.**

일차 시작 화면에는 현재 일차, 그날 밤의 습격 예고, 전날 일지를 둔다. 자동으로 넘어가지 않고
플레이어 입력으로 닫는다 — 일지를 읽을 시간이 필요하기 때문이다. 전체 습격 일정과 남은 일수는
보여주지 않고, 일차별 목표 시스템이 없으므로 목표 표시도 만들지 않는다.

일지는 일차 시작 화면 안에만 있고 별도 화면을 두지 않는다. 1일차 아침에는 영역 자체가 없다.
생성 중에는 대기 표시만 하고 폴백을 정상 생성분과 구분하지 않으며 다시 생성하는 입력도 두지
않는다. **엔딩에서 지난 일지를 회고로 보여주지 않는다.**

핵심 근거: `DEC-UI-032`, `DEC-UI-027`, `DEC-UI-016`, `DEC-UI-028`.

### 회복 퀵메뉴·보관함 표시·설정

회복 퀵메뉴는 `Q`를 누르고 있는 동안만 열린다. 보유 수량이 1개 이상인 항목만 나오고, 순서는
**제작 회복 아이템 → 생식 작물, 각 묶음 안에서 ID 오름차순**이다. 이 순서를 자동 선택 순서로도
쓴다 — `DEC-CONTENT-006`이 데이터에 정렬 열을 두지 못하게 확정해서 UI가 정해야 했다.

보관함에는 **수량이 1개 이상인 것만** 나온다. 수량 0이 되면 목록에서 빠지고, 획득한 적 없는
아이템을 미리 보여주지 않는다. 투척 퀵슬롯에 편성된 무기만 예외로 편성을 유지한 채 사용 불가로
표시한다.

일시정지 화면에는 계속하기, 음량(전체·배경음·효과음), 조작 안내, 타이틀 복귀를 둔다. 오디오를
구현하지 않는 빌드에서는 음량 항목을 표시하지 않는다. 타이틀로 돌아가면 현재 런이 사라지므로
확인을 받고, 별도의 런 재시작 입력은 두지 않는다.

핵심 근거: `DEC-UI-001`, `DEC-UI-003`, `DEC-UI-027`.

---

**폐기되지 않은 `DEC-UI-*` 30개와 아트 규격(`DEC-ART-005`)이 모두 확정됐다.** 남은 것은 화면 문구 테이블
두 종(`raid_notices`, `night_result_texts`)의 실제 값이다. 콘텐츠 CSV에 임의의 화면 좌표·정렬·
연출 값을 넣지 않는다.

### 아트 규격

기준 해상도는 1920×1080이고 **월드 1단위 = 화면 1픽셀**이다. 스프라이트의 기준점은 중심이며
논리 좌표를 그 중심에 맞춘다. 이미지는 PNG, 소리는 배경음(`bgm`) MP3·효과음(`sfx`) WAV,
폰트는 WOFF2를 쓰고, 1차 프로토타입에서는 **프레임 애니메이션과 스프라이트 시트를 쓰지
않는다.**

논리 에셋 ID는 `asset.<구간>.<이름>`이고 파일 경로는 카탈로그 없이
`assets/final/<구간>/<이름>.<확장자>`로 해석한다. 콘텐츠에 붙는 구간이 있고, 어떤 콘텐츠에도
속하지 않거나 한 콘텐츠에 같은 구간이 여럿 필요해 연결 CSV의 고유키와 부딪히는 UI·시스템
에셋(배경음 포함)은 `schema/enums.json`의 고정 허용 목록으로 관리한다. 해석되지 않는 ID는
플레이스홀더로 그린다.

팝업·모달·패널의 틀은 크기가 가변이라 고정 이미지를 쓰지 않고 반복 텍스처와 `border-image`
9-slice로 표현하며 화면 전체가 공유한다. 버튼만 기본·눌림·비활성 세 상태를 이미지로 둔다.
색·여백·그림자는 `layout.css`의 값이다.

게임용 결과물은 `assets/final/`, AI 생성 원본과 편집 전 파일은 `assets/source/`, 임시 표시는
`assets/placeholder/`에 둔다. 채택하지 않은 시안은 저장소에 누적하지 않는다.
...
다음은 구조가 아니라 실제 콘텐츠·밸런스 작업이다. 2026-08-05 기준 아래 값은 대부분 승인
데이터에 작성됐으며, 남은 것은 아트 리소스 자체의 제작이다. 공포도 증가량은
`DEC-RESIDENT-048`, 밤 결과 문구의 선택 방식은 `DEC-CONTENT-018`에서 확정했고
`fear_increments.csv`와 밤 결과 문구 목록의 승인이 남아 있다.
