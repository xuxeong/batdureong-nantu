...
전부 보여야 하며, 실제 월드 단위 대 화면 픽셀의 배율은 `DEC-ART-001`이 이 제약을 지키는
선에서 정한다.
...
**2026-08-03 기준 `DEC-UI-*` 28개가 모두 확정됐다.** 남은 것은 아트 규격(`DEC-ART-001`)과
화면 문구 테이블 두 종(`raid_notices`, `night_result_texts`)의 실제 값이다. 콘텐츠 CSV에 임의의
...
(`DEC-CONTENT-018`), 아트 규격(`DEC-ART-001`)이다.
